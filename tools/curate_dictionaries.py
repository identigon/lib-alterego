...
# --- Surnames --------------------------------------------------------------------------------
# All four UK nations, but not an equal-weight merge. Kept
# roughly representative instead by treating England & Wales's ONS list as the anchor (with two
# cohort years, 2024 and 2014, since it's by far the largest population) and adding only
# Scotland's top-10 (not top-20) for a single current year, so it contributes meaningfully
# without dominating. No Northern Ireland-specific surname source exists (ONS/NRS confirm
# neither publishes a broader list than these; no NISRA equivalent was found). Wales has no
# separate figure of its own — ONS reports "England and Wales" as one combined ranking.
# Close-to-representative, not exact: no precise population-proportional maths, just an anchor
# nation at full size plus smaller nations at reduced size.

ONS_SURNAMES_2024 = [
    "Smith", "Khan", "Ahmed", "Ali", "Jones", "Hussain", "Williams", "Singh", "Kaur", "Taylor",
    "Patel", "Brown", "Davies", "Thomas", "Evans", "Johnson", "Wilson", "Roberts", "Thompson",
    "Robinson",
]

ONS_SURNAMES_2014 = [
    "Smith", "Jones", "Williams", "Khan", "Ali", "Ahmed", "Hussain", "Taylor", "Brown", "Davies",
    "Evans", "Thomas", "Johnson", "Patel", "Wilson", "Roberts", "Robinson", "Wright", "Edwards",
    "Thompson",
]

NRS_SURNAMES_2025_TOP10 = [
    "Smith", "Brown", "Wilson", "Thomson", "Campbell", "Stewart", "Robertson", "Anderson",
    "Taylor", "Murray",
]

SURNAMES_SOURCE = (
    "Office for National Statistics: Twenty most common surnames for births in England and "
    "Wales, 2014 and 2024 (both years used); "
    "National Records of Scotland: Most common surnames, 2025 (top 10 only)"
)
SURNAMES_DATA_URL = (
    "https://www.ons.gov.uk/aboutus/transparencyandgovernance/freedomofinformationfoi/"
    "twentymostcommonsurnamesforbirthsinenglandandwales2014and2024; "
    "https://www.nrscotland.gov.uk/publications/most-common-surnames-2025/"
)
...
# --- Street themes -----------------------------------------------------------------------------
# Theme words from published frequency analyses of real UK street names
# (ultimately OS Open Names data, OGL v3.0).
#
# Words already used as street *types* (Green, Grove, Avenue, Crescent, Drive) are excluded
# here to avoid degenerate combinations like "Green Green".

STREET_THEMES = [
    # drivers.co.uk "Revealed: The UK's most common street names" (frequency data: High Street
    # 2086, London Road 4992, Station Road 1939, Church St/Lane/Road 1408+1328+1074, Mill Lane
    # 1032, Main Street 821, School Lane 803, Queen Street 560, Victoria Road 554) and
    # thefactsite.com "The Most Common Street Names In The UK" (50-entry list, no frequency data)
    "Alexander", "Albert", "Broadway", "Chester", "Church", "George", "Grange", "High",
    "Highfield", "King", "London", "Main", "Manor", "Mill", "New", "Park", "Queen", "Richmond",
    "School", "Springfield", "Stanley", "Station", "Victoria", "Windsor", "York",
]
STREET_THEMES_SOURCE = (
    "drivers.co.uk 'Revealed: The UK's most common street names' and thefactsite.com 'The Most "
    "Common Street Names In The UK' — both secondary reporting ultimately over OS Open Names "
    "frequency data, OGL v3.0"
)
STREET_THEMES_DATA_URL = (
    "https://drivers.co.uk/general/revealed-the-uks-most-common-street-names/; "
...
def main() -> None:
    ...
    write_flat(
        ...
        SURNAMES_SOURCE,
        SURNAMES_DATA_URL,
        ONS_SURNAMES_2024 + ONS_SURNAMES_2014 + NRS_SURNAMES_2025_TOP10,
    )
    write_flat(
        "first-names",
        FIRST_NAMES_SOURCE,
    ...
    write_flat("street-themes", STREET_THEMES_SOURCE, STREET_THEMES_DATA_URL, STREET_THEMES)
